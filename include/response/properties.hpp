...
namespace ChronusQ {
  ...
  void ResponseTBase<T>::fdrRF(FDResponseResults<T,U> &results) {
    ...
    // Electric Quadrupole - Electric Quadrupole Polarizability (Len)
    if( AHasEQL and BHasEQL ) {


      results.eq_eq_Polar =
        CQMemManager::get().malloc<U>(6*6*nOmega);

      for(auto iOmega = 0; iOmega < nOmega; iOmega++) {

        SetMat('N',6,6,U(1.),
          opMap[LenElectricQuadrupole] + eqlOff + nRHS*iOmega,
      ...
      if( savFile.exists() )
        savFile.safeWriteData("/RESP/FDR/EQ_EQ_POLARIZABILITY_LENGTH",
          results.eq_eq_Polar, {nOmega,6,6});
